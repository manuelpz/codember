...
function contarCadena(palabraCompleta) {
  let animalesUnicos = []
  let resultado = ''
  let cont = 0
  for (let animal of palabraCompleta.split(' ')) {
    if (!animalesUnicos.includes(animal)) {
      animalesUnicos.push(animal)
    }
  }

  for (let animales of animalesUnicos) {
    cont = 0
    for (let animal of palabraCompleta.split(' ')) {
      if (animales && animales == animal) {
        cont++
      }
    }
    resultado += animales + cont
  }
  return resultado
}
console.log(contarCadena(palabraCompleta))
